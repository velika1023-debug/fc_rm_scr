...
##### rm_generate_variables_for_report_parallel_hpc
proc rm_generate_variables_for_report_parallel_hpc { args } {
  ...
  set GLOBAL_VARS [list \
    HPC_CORE \
    CURRENT_STEP \
    REPORT_PREFIX \
    REPORT_PARALLEL_MAX_CORES \
    SEV \
    TCL_USER_NON_PERSISTENT_SCRIPT \
    TCL_USER_SUPPLEMENTAL_REPORTS_SCRIPT \
    ENABLE_FUSA \
    REPORT_STAGE REPORT_ACTIVE_SCENARIOS REPORT_DISABLE_GUI \
  ]
  global {*}$GLOBAL_VARS
  ...
  if {[info exists REPORT_PREFIX]} {
    ...
    ## Source files and define variables identical to the run scripts
    puts $fid "source [pwd]/rm_utilities/procs_global.tcl"
    if {$::synopsys_program_name == "fc_shell"} {
      puts $fid "source [pwd]/rm_utilities/procs_fc.tcl"
    } else {
    ...
    puts $fid "rm_source -file [pwd]/rm_setup/design_setup.tcl"
    puts $fid "rm_source -file [pwd]/rm_setup/fc_setup.tcl"
    puts $fid "rm_source -file [pwd]/rm_setup/header_fc.tcl"
    puts $fid "rm_source -file sidefile_setup.tcl -after_file technology_override.tcl"
    if {$HPC_CORE != ""} {
      puts $fid "rm_source -file [pwd]/flow_override.tcl"
      puts $fid "rm_source -file [pwd]/rm_hpc_core_scripts/design_config.tcl"
      # Restore to REPORT_PARALLEL_MAX_CORES since sourcing rm_hpc_core_scripts/design_config.tcl will set max_cores to NUM_CORES
      puts $fid "set_host_options -max_cores $REPORT_PARALLEL_MAX_CORES"
    }

    puts $fid "## RM Tcl variables required by report_parallel"
    puts $fid "set CURRENT_STEP $CURRENT_STEP"
    puts $fid "set REPORT_PREFIX $REPORT_PREFIX"
    puts $fid "set REPORT_STAGE $REPORT_STAGE"
    puts $fid "set REPORT_ACTIVE_SCENARIOS \"$REPORT_ACTIVE_SCENARIOS\""

    if {$HPC_CORE != ""} {
      puts $fid "rm_source -file \$TCL_USER_NON_PERSISTENT_SCRIPT -optional -print \"TCL_USER_NON_PERSISTENT_SCRIPT\""
      puts $fid "if {\$HPC_CORE != \"\"} {rm_source -file \${HPC_PLUGINS_DIR}/non_persistent_script.tcl -print \"HPC_NON_PERSISTENT_SCRIPT\"; set CURRENT_PLUGIN_STEP \"non_persistent\"; source \$HPC_USER_OVERRIDES_SCRIPT}"
    }
...
define_proc_attributes rm_generate_variables_for_report_parallel_hpc \
        -info "Generate and pass necessary RM variables for the report_parallel command" \
   	-define_args {
  	{-work_directory "Destination directory for the output file." AString string optional}
  	{-file_name "Name of the output file." AString string required}
...
proc rm_check_macro_placement {} {
  ...
  set unplaced_macro_cells [get_cells -quiet -hierarchical -physical_context -filter "(is_memory_cell==true || is_hard_macro==true || design_type==macro) && physical_status==unplaced"] ;
...
proc rm_check_design { args } {
  ...
  if {[current_design] != ""} {
	 ...
	 if {[sizeof_collection [check_duplicates -return_as_collection]] > 0} {
	 	 puts "RM-warning: Design has duplicate shapes that could not be removed by 'check_duplicates -remove'. If this is unexpected, please remove them before you continue!"
	 }
