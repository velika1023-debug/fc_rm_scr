        ...
        chomp $line;
		  ...
		  if ($script_error_count == 0 && $script_stop == 0) {  #Only print the SCRIPT_START line once
		   ...
		   $script_error_count++;
			...
			print "\t$line issued on line $.\n";
		  } 
	          $script_error_count++;	
		  if ($script_error_count == 21 && $script_stop == 0) {
			print "\tERROR LIMIT REACHED, see log for additional error messages\n";
		  } elsif ($script_stop != 0) { 
			$tool_error_count++; 
			if ($tool_error_count > 0 && $tool_error_count <= 20) { 
			     print "Tool $line issued on line $.\n"; #Print detected Errors not related to rm_source 
		        } 
			if ($tool_error_count == 21) { print "ERROR LIMIT REACHED, see log for additional error messages\n";} 
		  }
...
sub parse_rm {
    ...
    while (my $line = <$fd>) {
        chomp $line;
              if($line =~ m/^RM-error/i) {
			print "$line issued on line $.\n";
			$error = 1; 	
                }
              if($line =~ m/^RM-warning/i) {
                        print "$line issued on line $.\n";
			$warning = 1;
                }
    }
     if($error == 0 && $warning == 0) {
		print "No RM-error or RM-warning messages detected in log file\n";
      }
}
...
for my $log (@files) {
   ...
   if ($errors_detected == 0) {print "No tool issued errors detected in log file\n";}
