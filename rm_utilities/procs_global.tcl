  ...
  foreach spec $file_spec {
    ...
    if { [llength $file_root] > 0 } {
      ...
      if { [file exists $script_file] } {
        ...
        if {$optional} {
          if { $print_string != "" } {
            puts "RM-warning   : rm_source: The specified optional file does not exist; '$file_root' : '$print_string'"
          } else {
            puts "RM-warning   : rm_source: The specified optional file does not exist; '$file_root'"
          }
        } else {
          if { $print_string != "" } {
...
define_proc_attributes rm_source \
  ...
  {-optional "Allows an <empty-string> -file argument or nonexistent file to output a message and not an error" "" boolean optional}
