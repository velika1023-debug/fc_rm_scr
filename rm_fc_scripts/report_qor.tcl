...
## global timing 
redirect -tee -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_global_timing {report_global_timing -enable_multiple_categories_per_endpoint -pba_mode [get_app_option_value -name time.pba_optimization_mode] -nosplit}
...
redirect -compress -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_constraint.gz {report_constraint -all_violators -max_transition -max_capacitance -scenarios [get_scenarios -filter active] -nosplit}
...
if {[regexp synthesis|mapped $REPORT_STAGE]} {
        ...
        set dynamic_power_scenarios [get_scenarios -filter "dynamic_power==true"]
        if { [sizeof_collection $dynamic_power_scenarios] > 0 } {
		redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_activity.rtl {report_activity -rtl -scenarios $dynamic_power_scenarios}
		redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_activity.driver.end {report_activity -driver -scenarios $dynamic_power_scenarios}
	} else {
		puts "Design has no scenarios with dynamic_power analysis enabled. Skipping activity reporting."
	}
}
if {![regexp init_design|synthesis|mapped $REPORT_STAGE]} {
        set dynamic_power_scenarios [get_scenarios -filter "dynamic_power==true"]
        if { [sizeof_collection $dynamic_power_scenarios] > 0 } {
		redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_activity.preplace {report_activity -verbose -scenarios $dynamic_power_scenarios}
	} else {
		puts "Design has no scenarios with dynamic_power analysis enabled. Skipping activity reporting."
	}
}
	...
	if { [sizeof_collection $power_scenarios] > 0 } {
	  ...
	  if {$REPORT_POWER_SAIF_FILE != ""} {
		...
		puts "RM-info: Running set_power_clock_scaling -ratio $REPORT_POWER_SCALING_RATIO"
	      ...
	      ## report the amount of annotation that was propagated on dynamic_power scenarios
	      set dynamic_power_scenarios [get_scenarios -filter "dynamic_power==true"]
	      redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_activity.end {report_activity -driver -show_zeros -verbose -scenarios $dynamic_power_scenarios}
...
if {[regexp init_design $REPORT_STAGE]} {
        ...
        redirect -tee -file ${REPORTS_DIR}/${REPORT_PREFIX}/report_supply_sets {report_supply_sets -nosplit}
}
...
if {[regexp init_design $REPORT_STAGE]} {
        redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/check_design.pre_placement_stage {check_design -ems_database $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_placement_stage.ems -checks pre_placement_stage -log_file $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_placement_stage.log} ; #
...
if {[regexp synthesis|placement $REPORT_STAGE]} {
    redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/check_design.pre_clock_tree_stage {check_design -ems_database $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_clock_tree_stage.ems -checks pre_clock_tree_stage -log_file $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_clock_tree_stage.log}
}
...
if {[regexp post_cts_opt $REPORT_STAGE]} {
        redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/${DESIGN_NAME}.check_design.pre_route_stage  {check_design -ems_database $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_route_stage.ems -checks pre_route_stage -log_file $REPORTS_DIR/$REPORT_PREFIX/check_design.pre_route_stage.log}
   }
...
##	report_ccd_timing -type fanin -pin <pin>
    ...
    puts "RM-info: Creating HPC reports with HPC_REPORT_QOR_SCRIPT: ${HPC_UTILITIES_DIR}/hpc_report_qor.tcl"
    rm_source -file ${HPC_UTILITIES_DIR}/hpc_report_qor.tcl -optional -print HPC_REPORT_QOR_SCRIPT
} else {
    puts "RM-error: REPORT_PREFIX is not specified. Exiting after sourcing optional TCL_USER_SUPPLEMENTAL_REPORTS_SCRIPT."
}

########################################################################
## TCL_USER_SUPPLEMENTAL_REPORTS_SCRIPT
########################################################################
