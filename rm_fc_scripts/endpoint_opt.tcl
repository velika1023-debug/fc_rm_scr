...
##---------------------------------
if {[info exists env(ENDPOINT_OPT_FROM_BLOCK_NAME)]} { set ENDPOINT_OPT_FROM_BLOCK_NAME $env(ENDPOINT_OPT_FROM_BLOCK_NAME) }
if {[info exists env(ENDPOINT_OPT_BLOCK_NAME)]}      { set ENDPOINT_OPT_BLOCK_NAME      $env(ENDPOINT_OPT_BLOCK_NAME) }
...
#####################################################################################
## Activate endpoint_opt scenarios. HPC cores can have different active scenarios for 
## endpoint_opt vs endpoint_opt -flat
#####################################################################################
if {$HPC_CORE != "" } {
	if {$ENDPOINT_OPT_BLOCK_NAME == "endpoint_opt_flat"} {
		if {$ENDPOINT_OPT_FLAT_ACTIVE_SCENARIO_LIST != ""} {
			set_scenario_status -active false [get_scenarios -filter active]
			set_scenario_status -active true $ENDPOINT_OPT_FLAT_ACTIVE_SCENARIO_LIST
		}
	} else {
...
## Prefix
set_app_options -name opt.common.user_instance_name_prefix -value ${ENDPOINT_OPT_BLOCK_NAME}_
set_app_options -name cts.common.user_instance_name_prefix -value ${ENDPOINT_OPT_BLOCK_NAME}_cts_
...
##########################################################################################
if {$HPC_CORE != ""} {
	## Support using a different pre script for endpoint_opt_flat
	if {$ENDPOINT_OPT_BLOCK_NAME == "endpoint_opt_flat"} {
		rm_source -file ${HPC_PLUGINS_DIR}/endpoint_opt_flat_pre_script.tcl -print "HPC_ENDPOINT_OPT_FLAT_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "endpoint_opt_flat_pre"; source $HPC_USER_OVERRIDES_SCRIPT
	} else {
		rm_source -file $TCL_USER_ENDPOINT_OPT_PRE_SCRIPT -optional -print "TCL_USER_ENDPOINT_OPT_PRE_SCRIPT"
		rm_source -file ${HPC_PLUGINS_DIR}/endpoint_opt_pre_script.tcl -print "HPC_ENDPOINT_OPT_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "endpoint_opt_pre"; source $HPC_USER_OVERRIDES_SCRIPT
	}
} else {
	rm_source -file $TCL_USER_ENDPOINT_OPT_PRE_SCRIPT -optional -print "TCL_USER_ENDPOINT_OPT_PRE_SCRIPT"
}
...
if {$HPC_CORE != ""} {
	...
	if {$ENDPOINT_OPT_BLOCK_NAME == "endpoint_opt_flat"} {
		rm_source -file ${HPC_PLUGINS_DIR}/endpoint_opt_flat_post_script.tcl -optional -print "HPC_ENDPOINT_OPT_FLAT_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "endpoint_opt_flat_post"; source $HPC_USER_OVERRIDES_SCRIPT
	} else {
		rm_source -file $TCL_USER_ENDPOINT_OPT_POST_SCRIPT -optional -print "TCL_USER_ENDPOINT_OPT_POST_SCRIPT"
		rm_source -file ${HPC_PLUGINS_DIR}/endpoint_opt_post_script.tcl -optional -print "HPC_ENDPOINT_OPT_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "endpoint_opt_post"; source $HPC_USER_OVERRIDES_SCRIPT
	}
...
if {$REPORT_QOR} {
        ...
        if {$HPC_CORE != ""} {
          ## Support using a different active scenario list for endpoint_opt_flat
          if {$ENDPOINT_OPT_BLOCK_NAME == "endpoint_opt_flat"} {
		set REPORT_ACTIVE_SCENARIOS $REPORT_ENDPOINT_OPT_FLAT_ACTIVE_SCENARIO_LIST
          } else {
...
rm_logparse $LOGS_DIR/${ENDPOINT_OPT_BLOCK_NAME}.log
echo [date] > $ENDPOINT_OPT_BLOCK_NAME
