...
if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/redhawk_sc_pre_script.tcl -optional -print "HPC_REDHAWK_SC_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "redhawk_sc_pre"; source $HPC_USER_OVERRIDES_SCRIPT}
...
if {[file exists [which $REDHAWK_SC_DIR]]} {
...
if {$REDHAWK_LIB_FILES == ""} {
	...
	rm_source -file $REDHAWK_LIB_FILES -print "REDHAWK_LIB_FILES"
} else {
...
if {[file exists [which $REDHAWK_PAD_FILE_NDM]]} {
	...
	rm_source -file $REDHAWK_PAD_CUSTOMIZED_SCRIPT -print "REDHAWK_PAD_CUSTOMIZED_SCRIPT"
} else {
...
if {$RHSC_PYTHON_SCRIPT_FILE == "" && $REDHAWK_MCMM_SCENARIO_CONFIG == ""} {
	set analyze_rail_cmd "analyze_rail -nets [list $REDHAWK_ANALYSIS_NETS] -voltage_drop $REDHAWK_ANALYSIS"
	
	if {$REDHAWK_EFFECTIVE_RESISTANCE} {
		lappend analyze_rail_cmd "-effective_resistance"
	}
	
	if {$REDHAWK_MIN_PATH_RESISTANCE} {
		lappend analyze_rail_cmd "-min_path_resistance"
	}
	
	if {$REDHAWK_CHECK_MISSING_VIA} {
		lappend analyze_rail_cmd "-check_missing_via"
	}
	
	if {$REDHAWK_USE_FC_POWER} {
		lappend analyze_rail_cmd "-power_analysis"
		lappend analyze_rail_cmd "icc2"
	}
	
	if {$REDHAWK_SWITCHING_ACTIVITY_FILE != ""} {
		lappend analyze_rail_cmd "-switching_activity $REDHAWK_SWITCHING_ACTIVITY_FILE"
	}
	
	if {$REDHAWK_EM_ANALYSIS} {
		lappend analyze_rail_cmd "-electromigration"
	}
	
	if {$REDHAWK_ADDITIONAL_ANALYZE_RAIL_OPTIONS != ""} {
		lappend analyze_rail_cmd "$REDHAWK_ADDITIONAL_ANALYZE_RAIL_OPTIONS"
	}
	
	puts "RM-info: Running: $analyze_rail_cmd"
	eval $analyze_rail_cmd
} else {
		...
		set_app_options -name rail.enable_parallel_run_rail_scenario -value true
        	...
        	rm_source -file $REDHAWK_MCMM_SCENARIO_CONFIG -print "REDHAWK_MCMM_SCENARIO_CONFIG"
        } else {
...
if {$HPC_CORE != ""} {
  rm_source -file ${HPC_PLUGINS_DIR}/redhawk_sc_post_script.tcl -optional -print "HPC_REDHAWK_SC_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "redhawk_sc_post"; source $HPC_USER_OVERRIDES_SCRIPT
