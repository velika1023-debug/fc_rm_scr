...
if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/init_design_pre_script.tcl -print "HPC_INIT_DESIGN_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "init_design_pre"; source $HPC_USER_OVERRIDES_SCRIPT}
...
if {[file exists $DESIGN_LIBRARY]} {
	if {$INIT_DESIGN_INPUT == "ASCII" && !$NETLIST2GDS_FLOW && [info exists INCREMENTAL_INIT_DESIGN]} {
...
if {$INIT_DESIGN_INPUT == "RTL"} {
	...
	if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/read_rtl_pre_script.tcl -optional -print "HPC_READ_RTL_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "read_rtl_pre"; source $HPC_USER_OVERRIDES_SCRIPT}		
	...
	switch ${RTL_SOURCE_FORMAT} {
        	...
        	script {if {[rm_source -file $FC_RTL_READ_SCRIPT -print "FC_RTL_READ_SCRIPT"]} {
				if {$SET_QOR_STRATEGY_MODE == "early_design"} {
					## Automatically enable lenient policy for early_design mode 
					set_early_data_check_policy -policy lenient -if_not_exist
				} elseif {$EARLY_DATA_CHECK_POLICY != "none"} {
			...
			if {[file exists $INIT_DESIGN_INPUT_LIBRARY] && $INIT_DESIGN_INPUT_BLOCK_NAME != ""} {
				...
				current_lib ${DESIGN_LIBRARY}
				current_block ${DESIGN_NAME}/${INIT_DESIGN_BLOCK_NAME}
				if {[get_attribute [get_designs] top_module_name] != $DESIGN_NAME} {          
					puts "RM-message: Setting the top module to be $DESIGN_NAME"   
					set_top_module $DESIGN_NAME
				}
			} else {
	...
	if {$RTL_SOURCE_FORMAT == "script" || $RTL_SOURCE_FORMAT == "elaborated_ndm"} {
		if {$SET_QOR_STRATEGY_MODE == "early_design"} {
			## Automatically enable lenient policy for early_design mode 
			set_early_data_check_policy -policy lenient -if_not_exist
		} elseif {$EARLY_DATA_CHECK_POLICY != "none"} {
	...
	if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/read_rtl_post_script.tcl -optional -print "HPC_READ_RTL_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "read_rtl_post"; source $HPC_USER_OVERRIDES_SCRIPT}
	...
	redirect -file ${REPORTS_DIR}/${REPORT_PREFIX}/check_design.design_mismatch {check_design -ems_database $REPORTS_DIR/$REPORT_PREFIX/check_design.design_mismatch.ems -checks design_mismatch -log_file $REPORTS_DIR/$REPORT_PREFIX/check_design.design_mismatch.log}
	...
	rm_source -file $DFT_PORTS_FILE -optional -print "DFT_PORTS_FILE"
        if {$HPC_CORE != ""} {rm_source -file ${HPC_UTILITIES_DIR}/connect_ema_ram_pins.tcl -optional -print "HPC_EMA_RAM_PINS_SCRIPT"}
	rm_source -file $TCL_USER_CREATE_DFT_PORTS_POST_SCRIPT -optional -print "TCL_USER_CREATE_DFT_PORTS_POST_SCRIPT"
	if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/create_dft_ports_post_script.tcl -optional -print "HPC_CREATE_DFT_PORTS_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "create_dft_ports_post"; source $HPC_USER_OVERRIDES_SCRIPT}
	
	################################################################
	## Read and commit the UPF file(s)  
	################################################################
        if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/load_upf_pre_script.tcl -print "HPC_LOAD_UPF_PRE_SCRIPT"; set CURRENT_PLUGIN_STEP "load_upf_pre"; source $HPC_USER_OVERRIDES_SCRIPT}
...
# Source after the design is created
...
if {$HPC_CORE != ""} {
  ...
  ################################################################
  ## Setup Required Prior to read_def
  ################################################################
  ## (Optional) source user provided via ladder definitions, if not defined in your technology file. The may be required for via definitions that are used by the DEF.
  rm_source -file $TCL_VIA_LADDER_DEFINITION_FILE -optional -print "TCL_VIA_LADDER_DEFINITION_FILE"
}

if {$UPF_FILE == ""} {
        puts "RM-warning: In case of non-MV designs, if there is no PG defined in the DEF, resolve_pg_nets/connect_pg_nets will automatically use VDD/VSS"
        puts "RM-warning: To change default PG net and port names use the following app options - mv.pg.default_power_supply_net_name, mv.pg.default_power_supply_port_name, mv.pg.default_ground_supply_net_name, mv.pg.default_ground_supply_port_name"
}
...
if {$INIT_DESIGN_INPUT == "ASCII" || $INIT_DESIGN_INPUT == "RTL"} {
	...
	if {$TCL_FLOORPLAN_FILE != ""} {
	        rm_source -file $TCL_FLOORPLAN_FILE -print "TCL_FLOORPLAN_FILE"
	...
	#  Script first checks if all the specified DEF files are valid, if not, read_def is skipped
	        ...
	        if {!$RM_DEF_FLOORPLAN_FILE_is_not_found} {
                  ...
                  eval ${read_def_cmd}
	                ...
	                if {$HPC_CORE != ""} {
               		  save_block -as ${DESIGN_NAME}/${CURRENT_STEP}_read_def
                  	  rm_source -file ${HPC_PLUGINS_DIR}/read_def_post_script.tcl -optional -print "HPC_READ_DEF_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "read_def_post"; source $HPC_USER_OVERRIDES_SCRIPT
                	}
...
if {$PARASITIC_TECH_LIB == "" } {
	...
	rm_source -file $TCL_PARASITIC_SETUP_FILE -optional -print "TCL_PARASITIC_SETUP_FILE"
        if {$HPC_CORE != ""} {rm_source -file ${HPC_PLUGINS_DIR}/parasitic_setup_file.tcl -optional -print "HPC_PARASITIC_SETUP_FILE"; set CURRENT_PLUGIN_STEP "parasitic_setup"; source $HPC_USER_OVERRIDES_SCRIPT}
}
...
if {![file exists header_from_dprm.tcl]} {
	rm_source -file $TCL_CTS_NDR_RULE_FILE -optional -print "TCL_CTS_NDR_RULE_FILE"
} else {
	puts "RM-info: Skipping sourcing of $TCL_CTS_NDR_RULE_FILE since it's already been sourced during design planning."
}
...
if {$HPC_CORE != ""} {
  rm_source -file ${HPC_PLUGINS_DIR}/init_design_post_script.tcl -optional -print "HPC_INIT_DESIGN_POST_SCRIPT"; set CURRENT_PLUGIN_STEP "init_design_post"; source $HPC_USER_OVERRIDES_SCRIPT
