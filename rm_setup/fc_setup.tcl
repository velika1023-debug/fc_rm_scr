...
## For DPS
set ENABLE_DPS				false ;# false|true; RM default false; set it to true for set_qor_strategy command to enable dynamic power shaping (DPS);
					;# only used by set_qor_strategy command which is part of the RM scripts and not used by RM scripts directly.
...
## For hyper_route_opt
set PBA_OPTIMIZATION_MODE		path ;# none|path|exhaustive|""; RM default path; controls the PBA optimization mode for hyper_route_opt; 
                                        ;# none disables PBA (sets time.pba_optimization_mode to none), path enables PBA path (sets time.pba_optimization_mode to path), exhaustive enables PBA exhaustive (sets time.pba_optimization_mode to exhaustive), and "" (null) does not change current PBA setting
