...
set INIT_DESIGN_HPC_BLOCK_NAME          "init_design_hpc"               ;# Label name to be used when saving a block in init_design.hier_hpc.tcl
...
set ABSTRACT_TYPE_FOR_MPH_BLOCKS "flattened" ;# "nested | flattened", Default flattened. Specifies the type of abstract to be created for MPH blocks (blocks with more than 1 level of physical hierarchy)
...
## FUSION_REFERENCE_LIBRARY_FRAM_LIST and FUSION_REFERENCE_LIBRARY_LEF_LIST are mutually exclusive. Only one of them shoould be used.
## Specify either frame & db as inputs (FUSION_REFERENCE_LIBRARY_FRAM_LIST & FUSION_REFERENCE_LIBRARY_DB_LIST), 
## or lef & db as inputs (FUSION_REFERENCE_LIBRARY_LEF_LIST & FUSION_REFERENCE_LIBRARY_DB_LIST)
...
set FUSION_REFERENCE_LIBRARY_LEF_LIST   [list ] ;# a list of LEF files for on-the-fly fusion reference library creation; ex : "[glob /path1/std/lef/*.lef /path2/sram/lef/*.lef]"
...
set LINK_LIBRARY			""	;# Optional; specify a list of .db files
...
set ENABLE_PERFORMANCE_VIA_LADDER	false ;# false|true; RM default false; For set_qor_strategy -stage compile_initial or place_initial, if this is true and the design is 7nm or smaller,  
					;# the set_qor_strategy command sets route.auto_via_ladder.update_during_route, place.legalize.enable_via_ladder_checks, vl.flow.enable_convergence_flow to true. 
					;# It runs the setup_performance_via_ladder -effort high command, and sources a file output from the command. 
					;# For set_qor_strategy -stage post_cts_opto, if this is true and design is 7nm or smaller, opt.common.enable_via_ladder_insertion is set to true.
					;# For more details, refer to the man page of the set_qor_strategy command.
...
set TCL_PVT_CONFIGURATION_FILE		"" ;# Optional; specify a file for user customized set_pvt_configuration commands;
...
set RTL_SOURCE_FILES		{} ;# RTL files for RTL_SOURCE_FORMAT = verilog | sverilog | vhdl
...
set REDHAWK_EFFECTIVE_RESISTANCE        true ;# Optional. Performs effective resistance analysis on the specified power and ground nets.
                                             ;# It generates an effective resistance map after analyze_rail
set REDHAWK_MIN_PATH_RESISTANCE         true ;# Optional. Performs minimum path resistance analysis on the specified power and ground nets.
                                             ;# The results are saved under the current rail database directory
set REDHAWK_CHECK_MISSING_VIA           true ;# Optional. Performs missing via check
set REDHAWK_ADDITIONAL_ANALYZE_RAIL_OPTIONS "" ;# Optional. Add additional options during analyze_rail cmd
...
set ECO_OPT_ENGINE                      "PrimeClosure-Fusion" ;# Required by eco_opt to specify which tool is used to perform the fusion ECO. Valid values are "PrimeClosure-Fusion" and "pt". By default it will run PrimeClosure-Fusion.
...
set ECO_OPT_RECIPE_INFO			"" ;# Required for eco_opt to spec one or more types of fixes to perform. This variable can be a single type or a list of one or 
                                        ...
                                        ;# engine options are max_capacitance|max_transition|setup|hold.
...
set ECO_OPT_PBA_OPTIMIZATION_MODE 	"" ;# Optional. NULL|none|path|exhaustive; default is NULL.
					;#Allows user to select the PBA optimization mode for eco_opt. When left to NULL, eco_opt will run with whatever value the incoming db has for time.pba_optimization_mode.
					;# When set to none -  time.pba_optimization_mode will be set to none (eco_opt will run in GBA mode)
					;# When set to path -  time.pba_optimization_mode will be set to path (eco_opt will run in PBA path mode)
					;# When set to exhaustive -  time.pba_optimization_mode will be set to exhaustive (eco_opt will run in PBA exhaustive mode)					
set POST_ROUTE_EXTRACTION_MODE		"fusion_adv" ;# fusion_adv|in_design|none; default is fusion_adv; sets extract.starrc_mode to corresponding value for post-route optimization (hyper_route_opt and eco_opt);
					;# fusion_adv and in_design modes require starrc config file to be specified;
...
## The following variables apply when using a user provided ECO change file.
set ECO_CHANGE_FILE 			"" ;# The eco_opt mode (default) is run when not set.  When set, this points to the ECO change file to implement.
set ECO_MODE				"default" ;# Specify the preferred flow for the ECO run; default|freeze_silicon
					;# default: sources $ECO_CHANGE_FILE and place_eco_cells in MPI mode
					;# freeze_silicon: add_spare_cells, place_eco_cells, sources $ECO_CHANGE_FILE, and place_freeze_silicon
set ECO_DISPLACEMENT_THRESHOLD 	"10" ;# A float to specify the maximum displacement threshold value for 
...
set TCL_USER_ROUTE_OPT_POST_ECO_SCRIPT    "route_opt_post_eco_script.tcl" ;# An optional Tcl file for customizations between 2nd and 3rd Phase of optimization during hyper_route_opt.(for ex, customized secondary PG routing)
...
set REPORT_ROUTE_OPT_THO_ACTIVE_SCENARIO_LIST     "" ;# A subset of scenarios to be made active when generating reports for the route_opt_tho step
set REPORT_ROUTE_OPT_FLAT_ACTIVE_SCENARIO_LIST    "" ;# A subset of scenarios to be made active when generating reports for the route_opt_flat step
set REPORT_ENDPOINT_OPT_FLAT_ACTIVE_SCENARIO_LIST "" ;# A subset of scenarios to be made active when generating reports for the endpoint_opt_flat step
...
set REPORT_PARALLEL_SUBMIT_COMMAND 	{} ;# for parallel reporting; if specified, script uses job submission for report_qor.tcl
					...
					;# Please make sure to provide the submit command in {} and be mindful of prepending special characters with a single escape character in the command strings. Below are a few examples for the submit command : 
					;#	1. Using qsub (no special characters) : {qsub -cwd -P di -pe mt 4 -m n}
					;#	2. Using qsub (with special characters) : {qsub -P bnormal -cwd -V -N report_parallel -pe mt 3 -l \"mem_free=4000M\"}
					;#	3. Using bsub (With special characters) : {bsub -o /dev/null -P RM_SAMPLE_DESIGN -J RM_TEST -app batch_normal -n 4 -R 'rusage\[mem=6G\]'}
...
set REPORT_FLATTEN_ACTIVE_SCENARIO_LIST "" ;# A subset of scenarios to be made active when generating reports for the flatten step
