...
#   set_host_options -name special_block -submit_command "rsh" localhost
...
##########################################################################################
set SUB_BLOCK_REFS                    [list] ;# design names for each physical block (including black boxes) in the design;
                                             ;# this includes bottom and mid level blocks in a Multiple Physical Hierarchy (MPH) design
set SUB_INTERMEDIATE_LEVEL_BLOCK_REFS [list] ;# design reference names for mid level blocks only
set SUB_BB_BLOCK_REFS                 [list] ;# Black Box reference names 
...
#set SUB_BB_BLOCK_REFS "leon3s_bb"
#set SUB_BB_BLOCKS(leon3s_bb,area)        [list 1346051] ;
#set SUB_BB_BLOCKS(leon3s_bb,upf)         [list ${des_dir}/leon3s_bb.upf] ;
#set SUB_BB_BLOCKS(leon3s_bb,timing)      [list ${des_dir}/leon3s_bbt.tcl] ;
#set SUB_BB_BLOCKS(leon3s_bb,boundary)    { {x1 y1} {x2 y1} {x2 y2} {x1 y2} {x1 y1} } ;
...
set FAST_COMPILE_HPC_ACTIVE_SCENARIO_LIST ""		;# A subset of scenarios to be made active during HPC DP compile step;
				            		;# once set, the list of active scenarios is saved and carried over to subsequent steps;
set REPORT_FAST_COMPILE_HPC_ACTIVE_SCENARIO_LIST ""     ;# A subset of scenarios to be made active when generating reports for the fast_compile_hpc step
...
set DESIGN_PLANNING_HPC_ACTIVE_SCENARIO_LIST ""         ;# A subset of scenarios to be made active when generating reports for the design_planning_hpc step
set REPORT_DESIGN_PLANNING_HPC_ACTIVE_SCENARIO_LIST ""  ;# A subset of scenarios to be made active when generating reports for the design_planning_hpc step
